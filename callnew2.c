...
{   
    ...
    loading();
     while(true)
    {
    printf("\n\n\t\t\t\t<<<<Welcome to the Scotland Yard>>>>\n\n");
    printf("\n\n\t\t\t\t\tHere we hunt Criminals ! \n\n");
    sleep(1);
    //system("cls");
   
        printf("\n\n\t\t\t\t\t<><><>Main Menu<><><>\n\n");
        printf("\n\n\t\t\t1) Start the game\n");
        printf("\t\t\t2) What is Scotland Yard and How to play !\n");
        printf("\t\t\t3) How does it work !?\n");
        printf("\t\t\t4) Exit\n");
...
void How_to_play()
{   printf("\n\n\t\t\t\t<<<>>>What is Scotland Yard and How to play !<<<>>>\n\n ");
    sleep(1);
    printf("\n\n\n\n\t\t\t\tHelloooooooooo!\n");
    sleep(1);
    printf("\t\t\tMe Willie !\n");
    sleep(1);
    printf("\t\t\tMe Willie Trombone !\n");
    sleep(1);
    printf("\t\t\tScotland Yard ! , Have you ever heard this name ?\n");
    sleep(1);
    printf("\t\t\tIt means somewhere that the detectives work .\n");
    sleep(1);
    printf("\t\tAnyway , Here we have a matrix that you can call it the board of the game.\n");
    printf("\t\t\tand you just tell me the length and width of it.\n");
    sleep(1);
    printf("\t\t Thus we have some police stations the have their own policemen that try to catch the criminal , \n");
    sleep(1);
    printf("\t\tand you need to tell me the number of the police stations and the number of their policemen .!\n");
    sleep(1);
    printf("\t\tAnd i'll tell you the number of police and the criminal move .!\n");
    sleep(1);
    printf("\t\tYou know ITS NOT A GAME !!!!!!!\n\n");
    sleep(1);
...
void How_does_it_work()
{   printf("\n\n\t\t\t\t<<<>>> How does it work !? <<<>>>\n\n ");
    sleep(1);
    printf("\n\n\n\n\n\t\t\t\tHi again !\n");
    sleep(1);
    printf("\t\tIt's not really hard to undrestand how does it work .\n");
    sleep(1);
    printf("\t\tAt first polices and the criminal locate in random locations .\n");
    sleep(1);
    printf("\t\tthen the criminal move randomly and if the place of a police was close enough \n");
    sleep(1);
    printf("\t\tor his other policemates were close enough the police won't move randomly , he moves to surround the criminal.\n");
    sleep(1);
    printf("\t\tIn fact when the criminal surrounded it can only move to somewhere that a police is staying at ,\n");
    sleep(1);
    printf("\t\tSo he will be Captured !\n\n");
    printf("\t\t\tPress enter to get back to main menu  ");
    getchar();
    getchar();
    system("cls");
...
void ps_finder_check(int x[20])
{
    for(int i=0; i<w ; i++ )
    {
        for(int j=0 ; j<l ; j++)
        {
	        ...
	        float dtc=(float)sqrt( (int)pow(i-pre_criminal_x,2) + (int)pow(j-pre_criminal_y,2) );
...
{  
     int dir_x=pre_criminal_x-i;
     int dir_y=j-pre_criminal_y;
     int s=i;
     int y=j;
     if(dir_x>0)
     {
            ...
            {   ps_number=matrix[i][j];
                switch (walkieTalkie(i,j))
                {
                    case 1: //time to catch;
                     if(matrix[pre_criminal_x][pre_criminal_y]==-100)
                         { 
                       // SetColor(purple);
                        tempx=pre_criminal_x;
                        tempy=pre_criminal_y;      
                               }
                        break;
...
int criminal_judge(void)
{
   ...
   // printf(" i: %d , j: %d , tempx: %d , tempy: %d \n",criminal_x,criminal_y,tempx,tempy);
    ...
    if(matrix[criminal_x][criminal_y]!=0)
    {   if(criminal_y==tempy && criminal_x==tempx)
        {   
            return 0;
        }
        else
...
void game_play(void)
{
    printf("\t\t\tPlease enter the lenght of your game board (its better to be something like : 1<n<15 , Max is 20 ) :\n\t\t\t");
    scanf("%d",&l);
    printf("\t\t\tPlease enter the width of your game board (its better to be something like : 1<n<15 , Max is 20 ) :\n\t\t\t");
    scanf("%d",&w);
    printf("\t\t\tPlease enter the number of your Police Stations (its better to be something like : 1<n<5 , Max is 20 )\n\t\t\t");
    scanf("%d",&ps);
    ...
    {   sleep(2);
        ps_finder_cleaner(ps_finder);
        pre_criminal_x=criminal_x;
        pre_criminal_y=criminal_y;
        ps_finder_check(ps_finder);
        criminal_judge();
        police_judge(matrix);
        printer(matrix);
    if(matrix[criminal_x][criminal_y]!=0)
